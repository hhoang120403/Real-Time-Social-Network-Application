...
describe('Get', () => {
  ...
  describe('all', () => {
    it('should send success json response if users in cache', async () => {
      ...
      await GetProfileController.prototype.getAllUsers(req, res);
      expect(
  ...
  describe('profile', () => {
    it('should send success json response if user in cache', async () => {
      ...
      jest
        ...
        .mockResolvedValue(existingUser);
      await GetProfileController.prototype.getUserProfile(req, res);
  ...
  describe('profileAndPosts', () => {
    it('should send success json response if user in cache', async () => {
      ...
      await GetProfileController.prototype.getUserProfileAndPosts(req, res);
      expect(UserCache.prototype.getUserFromCache).toHaveBeenCalledWith(
        `${req.currentUser?.userId}`,
      );
      expect(PostCache.prototype.getUserPostsFromCache).toHaveBeenCalledWith(
        'post',
        parseInt(req.params.uId as string, 10),
      );
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
  ...
  describe('profileByUserId', () => {
    it('should send success json response if user in cache', async () => {
      ...
      await GetProfileController.prototype.getUserProfileById(req, res);
      expect(UserCache.prototype.getUserFromCache).toHaveBeenCalledWith(
        req.params.userId,
      );
      expect(res.status).toHaveBeenCalledWith(200);
  ...
  describe('randomUserSuggestions', () => {
    it('should send success json response if user in cache', async () => {
      ...
      await GetProfileController.prototype.randomUserSuggestions(req, res);
