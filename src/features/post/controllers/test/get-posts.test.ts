...
describe('Get', () => {
  ...
  describe('posts', () => {
    it('should send correct json response if posts exist in cache', async () => {
      ...
      jest
        .spyOn(PostCache.prototype, 'getTotalPostsFromCache')
        .mockResolvedValue(1);

      await GetPostsController.prototype.posts(req, res);
  ...
  describe('postWithImages', () => {
    it('should send correct json response if posts exist in cache', async () => {
      ...
      await GetPostsController.prototype.postsWithImages(req, res);
      expect(
