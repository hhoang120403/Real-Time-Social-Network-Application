...
describe('Get', () => {
  ...
  describe('userFollowing', () => {
    it('should send correct json response if user following exist in cache', async () => {
      ...
      await GetFollowersController.prototype.getUserFollowing(req, res);
      expect(
        FollowerCache.prototype.getFollowersFromCache,
      ).toHaveBeenCalledWith(`following:${req.currentUser?.userId}`);
  ...
  describe('userFollowers', () => {
    it('should send correct json response if user follower exist in cache', async () => {
      ...
      await GetFollowersController.prototype.getUserFollowers(req, res);
      expect(
        FollowerCache.prototype.getFollowersFromCache,
      ).toHaveBeenCalledWith(`followers:${req.params.userId}`);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
