...
const log: Logger = config.createLogger('message-cache');
...
export class MessageCache extends BaseCache {
  ...
  public async addChatListToCache(
    ...
    try {
      ...
      if (userChatList.length === 0) {
        ...
        const receiverIndex: number = findIndex(userChatList, (item: string) =>
  ...
  public async addChatUsersToCache(value: IChatUsers): Promise<IChatUsers[]> {
    try {
      ...
      const userIndex: number = findIndex(
        users,
        (item: IChatUsers) => JSON.stringify(item) === JSON.stringify(value),
      );
  ...
  public async updateChatMessage(
    ...
    try {
      ...
      if (index === -1 || !message) {
        return {} as IMessageData & { isLastMessage: boolean };
      }
  ...
  public async updateMessageReaction(
    ...
    try {
      ...
      const messageIndex: number = findIndex(messages, (item: string) =>
      ...
      if (parsedMessage) {
        remove(
          ...
          (item: IReaction) => item.senderName === senderName,
        );
        if (type === 'add') {
          reactions.push({
            senderName,
  ...
  private async getMessage(
    ...
    if (!parsedReceiver) {
      return { index: -1, message: '', receiver: {} as IChatList };
    }
    ...
    const index: number = findIndex(messages, (item: string) =>
      item.includes(messageId),
    );
    return {
      index,
